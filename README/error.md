...
## 9. grid 시 null 데이터를 공백으로 남기는 현상
- 광고 상품들을 `data.map` 으로 나열시 광고가 아닌 상품을 삼항 연산자로 null 처리를 했다.
- 이때 null 처리된 상품들의 공간이 생겨났다.

</br>

> 원인 & 해결
- 공간이 생겨난 이유는 div 안에 key 값을 줬었고, 그 안에서 삼항연산자로 null 처리를 했기 때문에 빈 div 가 발생 했었음
- 이를 해결하고자 삼항연산자로 우선 필터를 걸친 후 key 값을 부여함으로서 
